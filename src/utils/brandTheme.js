const DEFAULT_BRAND = '#0284c7';

function hexToRgb(hex) {
    const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex || DEFAULT_BRAND);
    if (!result) return { r: 2, g: 132, b: 199 };
    return {
...
export function applyBrandTheme(hex) {
    const color = /^#([A-Fa-f0-9]{6})$/.test(hex) ? hex : DEFAULT_BRAND;
