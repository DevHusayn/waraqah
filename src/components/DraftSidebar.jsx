...
export default function DraftSidebar({
    ...
    if (drafts.length === 0 && !compact) {
        return (
            <div className="card">
                <h3 className="text-sm font-semibold text-foreground mb-2">Your drafts</h3>
        ...
        <>
            ...
            <div className="card space-y-3">
                <div className="flex items-center justify-between gap-2">
                    <h3 className="text-sm font-semibold text-foreground">Your drafts</h3>
                    {drafts.length > 0 ? (
                ...
                {visibleDrafts.length === 0 ? (
                    <p className="text-sm text-foreground-muted">No other drafts saved yet.</p>
                ) : (
                    <ul className="space-y-3">
                        {visibleDrafts.map((draft) => {
                            ...
                            return (
                                <li
                                    key={draft.id}
                                    className="rounded-xl border border-border bg-surface-muted/60 p-3 space-y-2"
                                >
                                    <div>
