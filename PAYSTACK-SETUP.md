...
Waraqah Premium: **₦5,000/month**, auto-renewing via Paystack Subscription Plans.
...
## Step 3 — Subscription plan (₦5,000/month)
...
Use it to test logo upload and PDF branding without paying. Turn off `ALLOW_DEV_PLAN` in production.
...
6. Paystack will charge **₦5,000 every month** until the user cancels  

**Cancel auto-renewal:** Settings → Premium section → **Cancel auto-renewal** (Premium stays until `premiumUntil`).
...
3. Create the **live** plan (or set live `PAYSTACK_PLAN_CODE`).  
...
| Each month | Paystack charges ₦5,000; webhook extends Premium |
...
| Plan not found | Set `PAYSTACK_PLAN_CODE` in `.env` |
